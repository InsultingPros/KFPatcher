...
var config string sDead, sSpectator, sReady, sNotReady, sAwaiting;
var config bool bPerk, bHP, bKills;
var array<string> sCachedPlayersInfo;
var float fDelay;
...
function GetServerPlayers( out ServerResponseLine ServerState )
{
  ...
  for( C=Level.ControllerList; C != none; C=C.NextController )
  {
    ...
    if( (PRI != none) && !PRI.bBot && MessagingSpectator(C) == none )
    {
      ...
      // our new functions are a bit heavy, so limit its execution
      if (Level.TimeSeconds >= class'stubGT'.default.fDelay)
      {
        ...
        class'stubGT'.default.fDelay = Level.TimeSeconds + 3.0f;
      }
      else
